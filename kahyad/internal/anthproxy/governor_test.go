...
func TestCheckBeforeForwardBlocksAtTaskCeiling(t *testing.T) {
	...
	ledger := &fakeLedger{}
	g.RecordUsage(context.Background(), ledger, "trace1", "t_ceiling", "claude-sonnet-5",
		Usage{InputTokens: 500_000}, 1.0, "ok", 100, "")

	res := g.CheckBeforeForward("t_ceiling")
	if res.Allowed {
	...
	if res2 := g.CheckBeforeForward("t_other"); !res2.Allowed {
...
func TestCheckBeforeForwardBlocksAtDailyBudget(t *testing.T) {
	...
	ledger := &fakeLedger{}
	g.RecordUsage(context.Background(), ledger, "trace1", "t_budget", "claude-sonnet-5",
		Usage{InputTokens: 1000, OutputTokens: 1000}, 0.02, "ok", 50, "")

	res := g.CheckBeforeForward("t_budget")
	if res.Allowed {
...
func TestCheckBeforeForwardBlocksAtMonthlyBudget(t *testing.T) {
	...
	g.RecordUsage(context.Background(), nil, "trace1", "t_monthly", "claude-sonnet-5",
		Usage{InputTokens: 1}, 6.0, "ok", 10, "")

	res := g.CheckBeforeForward("t_monthly")
...
func TestCheckBeforeForwardAllowsUnderLimits(t *testing.T) {
	...
	res := g.CheckBeforeForward("t_fresh")
	if !res.Allowed {
...
func TestDowngradeFlipsAt80Percent(t *testing.T) {
	...
	g.RecordUsage(context.Background(), ledger, "trace1", "t1", "claude-opus-4-8",
...
func TestSpendAlarmsFireOnceAt80And100(t *testing.T) {
	...
	g.RecordUsage(context.Background(), nil, "trace1", "t1", "claude-sonnet-5", Usage{InputTokens: 1}, 8.5, "ok", 10, "")
	...
	g.RecordUsage(context.Background(), nil, "trace1", "t1", "claude-sonnet-5", Usage{InputTokens: 1}, 2.0, "ok", 10, "")
...
func TestCacheHitAlarmFiresBelowThresholdWith20Calls(t *testing.T) {
	...
	// >=20 calls).
	for i := 0; i < 19; i++ {
		g.RecordUsage(context.Background(), nil, "trace1", "t1", "claude-sonnet-5",
...
func TestCacheBusterSuspectAfterThreeChanges(t *testing.T) {
	...
	for _, h := range hashes {
		g.RecordUsage(context.Background(), ledger, "trace1", "t1", "claude-sonnet-5",
...
func TestModelCallAlwaysLedgered(t *testing.T) {
	...
	g.RecordUsage(context.Background(), ledger, "trace-xyz", "t1", "claude-sonnet-5",
...
func TestBootThenCheckBeforeForwardBlocksAlreadyAtCeiling(t *testing.T) {
	...
	res := g.CheckBeforeForward("t_old")
	if res.Allowed {
