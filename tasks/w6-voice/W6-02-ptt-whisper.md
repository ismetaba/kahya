...
**Status:** code-complete; live Hammerspoon hold-to-talk under launchd = user-assist (see docs/tcc-w6-checklist.md — same Microphone/Input Monitoring grants W6-01 already documents)
...
## Acceptance criteria
- [x] `make test` green, including `worker/tests/test_stt.py` — fixture transcript contains the byte-exact substring `toplantı` (no case folding — Turkish İ/ı is not `.lower()`-safe) with `HF_HUB_OFFLINE=1` (proves zero-network transcription). Verified: the live-model test actually RAN (not skipped) on this machine — `mlx-whisper` pinned in `worker/requirements.lock`, model present per W0-03.
- [x] `grep -n 'language="tr"' worker/kahya_worker/stt.py` returns a match, and `language` appears in no envelope/config schema (the §4 "sabit" clause). Verified by grep — see commit report.
- [x] `kahya ask --audio worker/tests/fixtures/tr_toplanti.wav` completes; `kahya log --trace <id>` shows an `stt.completed` JSONL line carrying the same `trace_id` as the kahyad task events. Verified LIVE against a real `KAHYA_ENV=dev` kahyad + real worker + real mlx-whisper (not just unit tests): `stt.completed chars=44 duration_ms=1415` appeared under the task's own `trace_id`, followed by `secretlane_classified`/`memory_search` on the transcript, proving the full downstream path runs unmodified. (The subsequent cloud call itself failed in this sandbox with "Invalid API key" — an unrelated Claude Code SDK credential limitation of the verification environment, not a defect in this task's own code path.)
- [x] After a live run: `ls ~/Library/Application\ Support/Kahya-dev/tmp/ | grep ptt-` is empty (temp audio deleted); the repo fixture still exists. Verified LIVE: copied the fixture into the dev-profile tmp dir as `ptt-livetest.wav`, ran `kahya ask --audio`, confirmed it was deleted afterward while `worker/tests/fixtures/tr_toplanti.wav` itself was untouched (`stt_delete_skipped_outside_tmp` logged for the repo-path case).
- [x] With the whisper model dir renamed away (simulated missing W0-03), `kahya ask --audio …` fails with the Turkish fail-closed error and **no** network download attempt (verify: no new files in the HF cache). Verified LIVE: renamed `~/.cache/huggingface/hub/models--mlx-community--whisper-large-v3-turbo` away, ran `kahya ask --audio` with `HF_HUB_OFFLINE=1`, got the byte-exact `STT modeli indirilmemiş (W0-03) — ağdan indirme yapılmadı`; `find` diff of the HF cache root before/after showed zero new files (only the deliberate rename itself); restored the directory afterward.
- [!] Manual, under launchd: hold `⌥Space`, say "yarın dokuzda toplantım var", release → `🎙️ Dinliyorum…` appeared while held; transcript-driven answer notification arrives; the whole capture+STT phase emits no egress-proxy log entries (`kahya log --trace <id>` shows the first proxy/model event only after `stt.completed`). — user-assist (needs a real Hammerspoon session with Microphone/Input Monitoring granted under launchd, not a terminal); `hammerspoon/kahya.lua`'s hold-to-talk binding is implemented per spec Step 5 (300ms `hs.timer`, `hs.task` ffmpeg capture, `task:terminate()` on release, `kahya ask --audio`) and the kahyad-side path is hermetically + live-verified above (this criterion is specifically about the physical hotkey/mic/notification UX, which needs the user).
- [x] W3-08 secret-lane tests still green: a spoken finance-flavored command routes exactly like the same typed command (no voice bypass of the ordering invariant). All pre-existing `kahyad/internal/secretlane`/`kahyad/internal/server` secret-lane tests pass unchanged; new regression `TestTaskInputAudioPathFinanceTranscriptRoutesSecretLaneNeverSpawnsChatWorker` (kahyad/internal/server/stt_task_test.go) proves an IBAN-bearing transcript is classified secret and the chat worker is never spawned; live-verified above too (the real fixture's ordinary transcript classified `lane=normal` via the identical `secretlane_classified` code path a typed prompt uses).
