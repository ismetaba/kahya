...
## Acceptance criteria
- [ ] `go test ./kahyad/internal/mlx/... ./kahyad/internal/secretlane/...` green in `make test` (model-dependent tests guarded by `KAHYA_MLX_TESTS=1`; deterministic pre-pass and fail-closed paths run unconditionally).
- [ ] Manual: `kahya "şu dosyayı özetle: ~/Documents/saglik/tahlil.md"` — JSONL logs show classifier verdict, `mlx_lm.server` spawn, ALL completion requests hitting `127.0.0.1:<qwen_port>`, zero requests at the Anthropic proxy for that `trace_id`; CLI output carries `🔒 yerel işlendi`.
- [ ] Fail-closed drill: with ComfyUI (or a memory-hog stub) consuming RAM so memcheck fails, the same command yields Turkish message `yerel model için bellek yok` and NO cloud call (grep proxy JSONL for the `trace_id` ⇒ zero hits).
- [ ] Proxy backstop test green: a secret-lane task's direct call to the forward-proxy returns 403 + `secretlane_cloud_blocked` ledger row — this is the §6 W3 gate "gizli-şerit içerik bulut çağrısına çıkamıyor (test)".
- [ ] Idle unload: after `idle_ttl`, `pgrep -f mlx_lm.server` is empty and a `mlx_unloaded` ledger event exists.
- [ ] Ordering invariant test: a content ingest whose classification is still pending/failed produces zero bytes at the proxy (test with a hanging classifier stub).
